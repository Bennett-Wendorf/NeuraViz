...
var app = (function () {
    ...
    function create_if_block(ctx) {
    	...
    	const block = {
    		c: function create() {
    			p = element("p");
    			t0 = text$1("Random number: ");
    			t1 = text$1(/*rand*/ ctx[0]);
    			add_location(p, file, 70, 8, 2235);
    		},
    		...
    		p: function update(ctx, dirty) {
    			if (dirty & /*rand*/ 1) set_data_dev(t1, /*rand*/ ctx[0]);
    		},
    ...
    function create_fragment(ctx) {
    	...
    	let mounted;
    	let dispose;
    	let if_block = /*rand*/ ctx[0] != -1 && create_if_block(ctx);
    	...
    	const block = {
    		c: function create() {
    			...
    			t4 = text$1(" to learn how to build Svelte apps.");
    			t5 = space();
    			br0 = element("br");
    			t6 = space();
    			...
    			t10 = space();
    			div = element("div");
    			attr_dev(h1, "class", "svelte-1qut6a6");
    			add_location(h1, file, 64, 4, 2000);
    			attr_dev(a, "href", "https://svelte.dev/tutorial");
    			add_location(a, file, 65, 17, 2039);
    			add_location(p, file, 65, 4, 2026);
    			add_location(br0, file, 66, 4, 2140);
    			add_location(button, file, 68, 4, 2152);
    			add_location(br1, file, 72, 4, 2278);
    			attr_dev(div, "id", "vis");
    			attr_dev(div, "class", "svelte-1qut6a6");
    			add_location(div, file, 74, 4, 2290);
    			attr_dev(main, "class", "svelte-1qut6a6");
    			add_location(main, file, 63, 0, 1989);
    		},
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = listen_dev(button, "click", /*getRand*/ ctx[2], false, false, false, false);
    				mounted = true;
    			}
    		},
    		p: function update(ctx, [dirty]) {
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	let getRand = () => {
    		api.get('/rand').then(res => res.data).then(data => $$invalidate(0, rand = data));
    	};
    	...
    	let xScale = linear().domain([0, 10]);
    	let yScale = linear().domain([0, 10]);
    	let width;
    	let height;
    	...
    	function redraw() {
    		// empty vis div
    		select(vis).html(null); // determine width & height of parent element minus the margin

    		width = select(vis).node().getBoundingClientRect().width - margin.left - margin.right;
    		height = select(vis).node().getBoundingClientRect().height - margin.top - margin.bottom; // init scales according to new width & height
    		xScale.range([0, width]);
    		yScale.range([height, 0]); // create svg and group that is translated by the margin
    		const svg = select(vis).append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', `translate(${[margin.left, margin.top]})`); // draw x and y axes
    		svg.append("g").attr("transform", `translate(${[0, height]})`).call(axisBottom(xScale));
    		svg.append("g").call(axisLeft(yScale)); // draw data points

    		svg.append('g').selectAll('circle').data(data).enter().append('circle').attr('cx', function (d) {
    			return xScale(d.x);
    		}).attr('cy', function (d) {
    			return yScale(d.y);
    		}).attr('r', 7).style('fill', '#ff3e00').style('fill-opacity', '0.5').style('stroke', '#ff3e00');
    	}
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ('vis' in $$props) $$invalidate(1, vis = $$props.vis);
    		if ('getRand' in $$props) $$invalidate(2, getRand = $$props.getRand);
    	...
    	return [rand, vis, getRand, div_binding];
    }
