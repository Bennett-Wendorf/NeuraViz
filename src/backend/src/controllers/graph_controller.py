...
positionless_nodes = [
    [
        Node(id = 0, test_value = 1),
        Node(id = 1, test_value = 1),
        Node(id = 2, test_value = 1),
        Node(id = 3, test_value = 1),
        Node(id = 4, test_value = 1),
    ], [
        Node(id = 5, test_value = 1),
        Node(id = 6, test_value = 1),
        Node(id = 7, test_value = 1),
        Node(id = 8, test_value = 1),
    ], [
        Node(id = 9, test_value = 1),
        Node(id = 10, test_value = 1),
        Node(id = 11, test_value = 1),
    ], [
        Node(id = 12, test_value = 1),
        Node(id = 13, test_value = 1),
    ]
]
...
def generate_links(nodes: List[List[Node]]) -> List[Link]:
    ...
    for layer in range(len(nodes) - 1):
        for node in range(len(nodes[layer])):
            links.extend([Link(source = nodes[layer][node].id, target = nodes[layer + 1][i].id) for i in range(len(nodes[layer + 1]))])
    return links
...
def position_nodes(nodes: List[List[Node]]) -> List[Node]:
    ...
    for index, layer in enumerate(nodes):
        ...
        for node_index, node in enumerate(layer):
            ...
            positioned_nodes.append(Node(id = node.id, test_value = node.test_value, x = layer_offset, y = node_offset))
