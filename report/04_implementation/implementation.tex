...
Currently, NeuraViz supports visualizing models created with Keras and PyTorch. As such, both of those frameworks are also included in the server portion of the application. When the graph endpoint is hit, the correct framework is determined based on the file extension of the uploaded file. From there, the framework is used to decode the model into a Python object, which is then parsed to retrieve the information needed for NeuraViz's visualization.
...
% TODO: Once info is added about framework background research, discuss why PyTorch was focused on first
